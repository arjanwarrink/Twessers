        ...
        <div class="wrap">
            ...
            <div id="openModal" class="apropos">
                <div>
                    ...
                    <h2><?php _e('About', BSCROLLTOP_TEXT_DOMAIN) ?></h2>
                    ...
                    <p><span class="dashicons dashicons-admin-site"></span> <?php _e('More information', BSCROLLTOP_TEXT_DOMAIN); ?> <a href="https://brozzme.com/" target="_blank">Brozzme</a></p>
                    <p><span class="dashicons dashicons-admin-tools"></span> <?php _e('Development for the web: WordPress plugins, theming, Php front & back-end developper...', BSCROLLTOP_TEXT_DOMAIN) ?></p>
                    <p><span class="dashicons dashicons-networking"></span> <?php _e('Contact slack: ', BSCROLLTOP_TEXT_DOMAIN) ?>@benoti</p>

                </div>
            </div>
            <h2><?= __('Plugins by Brozzme.', BSCROLLTOP_TEXT_DOMAIN); ?></h2>

            <p><?= __('A dedicated set of tools to improve your website. Focus on design and simply tricks.', BSCROLLTOP_TEXT_DOMAIN); ?></p>
            <?php

            require_once( BSCROLLTOP_DIR . '/includes/admin/remote_plugins.php');
                ...
                if (!empty($oPlugins->plugins['actif'])):
                    foreach ($oPlugins->plugins['actif'] as $plugin):
                        ...
                        <p id="seccion-<?= $plugin[0] ?>" style="border-left: 5px solid #0072aa;">
                            ...
                            <?= $plugin[1][0] . ' ' . __('[Latest version]', BSCROLLTOP_TEXT_DOMAIN) . ' - <a href="' . $plugin[1][1] . '">' . __('Plugin page', BSCROLLTOP_TEXT_DOMAIN) . '</a><br />'; ?>
                            <?= $plugin[1][2] ?>
                        </p>
                        <?php
                    endforeach;
                ...
                if (!empty($oPlugins->plugins['noInstall'])):
                    foreach ($oPlugins->plugins['noInstall'] as $plugin):
                        ...
                        <p id="seccion-<?= $plugin[0] ?>" style="border-left: 5px solid #32373c;">
                            ...
                            <?= $plugin[1][0] . ' - <a href="' . $plugin[1][1] . '">' . __('Plugin page', BSCROLLTOP_TEXT_DOMAIN) . '</a><br />' ?>
                            <?= $plugin[1][2] ?>
                        </p>
                        <?php
                    endforeach;
